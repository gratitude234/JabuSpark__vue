<template>
  <main class="relative min-h-screen bg-slate-50">
    ...
    <section class="mx-auto max-w-6xl px-4 py-6 sm:py-8 lg:py-10">
      ...
      >
        <div class="space-y-1">
          <p class="font-semibold">Couldn&apos;t load your dashboard</p>
      ...
      >
        <div class="space-y-1.5">
          ...
          <!-- Step 6: Quick context line -->
          <p class="text-[11px] text-slate-500" v-if="lastOpenedCourseId || lastDrill">
            <span v-if="lastOpenedCourseId">
              Last opened course:
              <span class="font-semibold text-slate-700">#{{ lastOpenedCourseId }}</span>
            </span>
            <span v-if="lastOpenedCourseId && lastDrill"> · </span>
            <span v-if="lastDrill">
              Last score:
              <span class="font-semibold text-slate-700">{{ lastDrill.score }}%</span>
            </span>
          </p>
        </div>
      ...
      <div class="grid gap-6 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)] xl:gap-8">
        ...
        <div class="space-y-6">
            ...
            <div class="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
              <div class="space-y-4">
                <div>
                  ...
                  <h2 id="hero-heading" class="mt-1 text-2xl font-semibold sm:text-3xl">
                    Stay consistent, {{ displayName }}.
                  </h2>
                  <p class="mt-2 max-w-md text-sm text-indigo-100/90">
                    Continue where you left off. If you haven&apos;t practiced yet, we&apos;ll take you
                    to a course so you can start immediately.
                  </p>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-3 text-xs">
                  <span class="inline-flex items-center gap-2 rounded-full bg-indigo-500/40 px-3 py-1 font-medium">
                    ...
                    <span>
                      ...
                      <span class="opacity-80"> · Longest {{ streak.longest }} days</span>
                    </span>
                  </span>

                  <span class="inline-flex items-center gap-2 rounded-full bg-sky-500/40 px-3 py-1 font-medium">
                    <span
                      class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-white/15 text-[10px] font-semibold"
                    >
                      🎯
                ...
                <div class="flex flex-wrap gap-3">
                  ...
                  <!-- Optional quick-start (kept as helpful shortcut) -->
                  <button
                    type="button"
                    class="inline-flex items-center justify-center gap-2 rounded-full border border-white/25 bg-white/10 px-4 py-2.5 text-sm font-semibold text-white backdrop-blur transition-all duration-200 hover:-translate-y-0.5 hover:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/80 focus-visible:ring-offset-2 focus-visible:ring-offset-indigo-600"
                    @click="startQuickDrill"
                    title="Start a new quick drill"
                  >
                    ⚡ New drill
                  </button>
                </div>
              ...
              <div class="w-full max-w-xs rounded-2xl bg-white/10 p-4 text-xs backdrop-blur-md shadow-inner">
                <p class="text-[11px] font-semibold uppercase tracking-wide text-indigo-100">
                  Today&apos;s snapshot
                </p>
                <div class="mt-3 space-y-2">
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm font-semibold text-white">{{ averageScore }}%</span>
                  </div>
                  <div class="flex items-center justify-between">
                    <span class="text-indigo-50">Courses this semester</span>
                    <span class="font-semibold text-white">{{ coursesCount }}</span>
                  </div>
                  <div class="flex items-center justify-between">
                    <span class="text-indigo-50">Questions available</span>
                    <span class="font-semibold text-white">{{ practiceQuestionsCount }}</span>
                  </div>

                  <!-- Step 6: last opened + last score -->
                  <div class="pt-2 mt-2 border-t border-white/15 space-y-1">
                    <p class="text-[11px] text-indigo-50">
                      Last opened:
                      <span class="font-semibold text-white">
                        {{ lastOpenedCourseId ? `#${lastOpenedCourseId}` : "—" }}
                      </span>
                    </p>
                    <p class="text-[11px] text-indigo-50">
                      Last score:
                      <span class="font-semibold text-white">
                        {{ lastDrill ? `${lastDrill.score}%` : "—" }}
                      </span>
                    </p>
                  </div>
                </div>
          ...
          >
            <header class="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h2 id="today-heading" class="text-sm font-semibold text-slate-900">
                  Today at a glance
                </h2>
            ...
            <div class="mt-4 grid gap-4 xs:grid-cols-2 sm:grid-cols-3">
              ...
              <div class="flex flex-col gap-2 rounded-2xl bg-slate-50 p-3 text-xs shadow-sm">
                <p class="text-xs font-semibold text-slate-700">This week’s drills</p>
                <p class="text-xs text-slate-500">Keep hitting small drills to maintain your streak.</p>
              ...
              <!-- Next assessment -->
              <div class="flex flex-col gap-2 rounded-2xl bg-slate-50 p-3 text-xs shadow-sm">
                <p class="text-xs font-semibold text-slate-700">Next assessment</p>
              ...
              <!-- Last activity -->
              <div class="flex flex-col gap-2 rounded-2xl bg-slate-50 p-3 text-xs shadow-sm">
                <p class="text-xs font-semibold text-slate-700">Last activity</p>
                ...
                <div v-if="lastDrill" class="space-y-1">
                  ...
                  <p class="text-[11px] text-slate-500">
                    {{ lastDrill.score }}% · {{ lastDrill.correct }}/{{ lastDrill.total }} correct
                  </p>
                  <p class="text-[11px] text-slate-500">{{ formatDateTime(lastDrill.date) }}</p>
                </div>

                <div v-else class="space-y-2">
                  <p class="text-[11px] text-slate-500">
                    No drills yet. Start your first quick drill to see progress here.
                  </p>
                  <button
                    type="button"
                    class="inline-flex w-fit items-center justify-center gap-2 rounded-full bg-indigo-600 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm transition hover:bg-indigo-700"
                    @click="continuePractice"
                  >
          ...
          >
            <header class="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h2 id="quick-actions-heading" class="text-sm font-semibold text-slate-900">
                  Quick actions
                </h2>
                <p class="mt-1 text-xs text-slate-500">Jump straight into the next best thing to do.</p>
              </div>
            </header>
            ...
            <div class="mt-4 grid grid-cols-2 gap-3 text-xs sm:grid-cols-4">
              ...
              >
                <div class="flex items-start gap-2">
                  <div class="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-50 text-[12px] font-semibold text-indigo-700">
                    ▶
                  </div>
                  <div class="space-y-1">
                    <p class="text-[12px] font-semibold text-slate-900">Continue practice</p>
                    <p class="text-[11px] text-slate-500">Resume your last course drill.</p>
                  </div>
                </div>
              </button>
              ...
              >
                <div class="flex items-start gap-2">
                  <div class="flex h-8 w-8 items-center justify-center rounded-full bg-sky-50 text-[12px] font-semibold text-sky-700">
                    ⚡
                  </div>
                  <div class="space-y-1">
                    <p class="text-[12px] font-semibold text-slate-900">New drill</p>
                    <p class="text-[11px] text-slate-500">5–10 questions to warm up.</p>
                  </div>
                </div>
              </button>
              ...
              >
                <div class="flex items-start gap-2">
                  <div class="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-50 text-[12px] font-semibold text-emerald-700">
                    📄
                  </div>
                  <div class="space-y-1">
                    <p class="text-[12px] font-semibold text-slate-900">Open materials</p>
                    <p class="text-[11px] text-slate-500">PDFs and notes for your courses.</p>
                  </div>
                </div>
              </button>
              ...
              >
                <div class="flex items-start gap-2">
                  <div class="flex h-8 w-8 items-center justify-center rounded-full bg-amber-50 text-[12px] font-semibold text-amber-700">
                    📚
                  </div>
                  <div class="space-y-1">
                    <p class="text-[12px] font-semibold text-slate-900">Pick a course</p>
                    <p class="text-[11px] text-slate-500">Choose where you want to study.</p>
                  </div>
                </div>
              </button>
            </div>
        ...
        <div class="space-y-6">
          ...
          >
            <header class="flex flex-wrap items-center justify-between gap-2">
              <h2 id="overview-heading" class="text-sm font-semibold text-slate-900">Study overview</h2>
              <p class="text-xs text-slate-500">Snapshot for this semester.</p>
            </header>

            <div class="mt-4 grid gap-3 xs:grid-cols-2 sm:grid-cols-3">
              <div class="flex flex-col gap-1 rounded-xl bg-indigo-50 p-3 text-xs shadow-sm">
                <p class="text-[11px] font-medium text-indigo-700">Courses this semester</p>
                <p class="mt-1 text-lg font-semibold text-slate-900">{{ coursesCount }}</p>
                <p class="text-[11px] text-slate-500">From your current enrolment.</p>
              </div>

              <div class="flex flex-col gap-1 rounded-xl bg-sky-50 p-3 text-xs shadow-sm">
                <p class="text-[11px] font-medium text-sky-700">Practice questions</p>
                <p class="mt-1 text-lg font-semibold text-slate-900">{{ practiceQuestionsCount }}</p>
                <p class="text-[11px] text-slate-500">Available across all your courses.</p>
              </div>

              <div class="flex flex-col gap-1 rounded-xl bg-emerald-50 p-3 text-xs shadow-sm">
                <p class="text-[11px] font-medium text-emerald-700">Average drill score</p>
                <p class="mt-1 text-lg font-semibold text-slate-900">{{ averageScore }}%</p>
                <p class="text-[11px] text-slate-500">Based on your recent sessions.</p>
              </div>
            </div>
          </section>

          <!-- Upcoming assessments (unchanged) -->
          <section
            class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/70 transition-all duration-500 ease-out transform"
            :class="loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3 delay-100'"
          ...
          >
            <header class="flex flex-wrap items-center justify-between gap-2">
              <h2 id="upcoming-heading" class="text-sm font-semibold text-slate-900">Upcoming assessments</h2>
              <p class="text-xs text-slate-500">The next few tests, quizzes, and exams.</p>
            </header>
            ...
            <div class="mt-4">
              ...
              >
                <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-600">Nothing scheduled yet</p>
              ...
              <ul v-else class="space-y-2 text-xs">
                ...
                >
                  <div>
                    <p class="text-[12px] font-semibold text-slate-900">{{ item.courseCode }} · {{ item.title }}</p>
                    <p class="mt-0.5 text-[11px] text-slate-500">
                      {{ item.date }} · {{ item.time }} · {{ item.type }}
                    </p>
                  </div>
                  <span class="mt-0.5 inline-flex items-center rounded-full bg-indigo-50 px-2 py-0.5 text-[11px] font-medium text-indigo-700">
          ...
          <!-- Recent drills (unchanged rendering, but Continue Practice now leads correctly) -->
          <section
            class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/70 transition-all duration-500 ease-out transform"
          ...
          >
            <header class="flex flex-wrap items-center justify-between gap-2">
              <h2 id="recent-heading" class="text-sm font-semibold text-slate-900">Recent drills</h2>
              <p class="text-xs text-slate-500">Latest practice sessions and scores.</p>
            </header>
            ...
            <div class="mt-4 text-xs">
              ...
              >
                <div>
                  <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-600">No drills yet</p>
                  <p class="mt-1 text-[12px] font-medium text-slate-900">
                ...
                >
                  <div class="flex flex-1 items-start gap-3">
                    ...
                    <div class="flex-1 space-y-1">
                      ...
                      <p class="text-[11px] text-slate-500">
                        {{ drill.correct }}/{{ drill.total }} correct · {{ drill.score }}% ·
          ...
          >
            <header class="flex items-center justify-between gap-2">
              <div>
                <h2 id="support-heading" class="text-sm font-semibold text-slate-900">
...
const props = defineProps({
  userName: { type: String, default: "Gratitude" },
...
const user = ref(null);
const stats = ref({ courses: 0, questions: 0, avg_score: null });
...
const streak = ref({ current: 0, longest: 0, this_week: 0, week_goal: 10 });

const level = ref("Student");
const weeklyDrills = ref({ current: 0, target: 10 });
...
// ✅ Step 6: last opened course (from localStorage)
const lastOpenedCourseId = computed(() => {
  const raw = localStorage.getItem("jabuspark_last_course_id");
  if (!raw) return null;
  const n = Number(raw);
...
// ---------- COMPUTED ----------
const displayName = computed(() => user.value?.name || props.userName || "Student");
const roleLabel = computed(() => user.value?.role || props.role || "Student");
...
const lastDrill = computed(() => (recentDrills.value.length ? recentDrills.value[0] : null));
...
function getPreferredCourseIdForDrill() {
  if (lastDrill.value?.courseId) return lastDrill.value.courseId;

  const firstCourse = activeCourses.value.find((c) => c.id);
  if (firstCourse) return firstCourse.id;

  // ✅ Step 6: fallback to last opened course
  if (lastOpenedCourseId.value) return lastOpenedCourseId.value;
...
async function loadDashboard() {
  ...
  try {
    ...
    averageScore.value = s.avg_score !== null && s.avg_score !== undefined ? Number(s.avg_score) : 0;

    if (data.meta) meta.value = { ...meta.value, ...data.meta };
    if (meta.value.level) level.value = meta.value.level;

    if (meta.value.completedDrillsThisWeek != null) weeklyDrills.value.current = meta.value.completedDrillsThisWeek;
    if (meta.value.targetDrillsThisWeek != null) weeklyDrills.value.target = meta.value.targetDrillsThisWeek;

    if (data.streak) {
      streak.value = { ...streak.value, ...data.streak };
      if (typeof data.streak.this_week === "number") weeklyDrills.value.current = data.streak.this_week;
      if (typeof data.streak.week_goal === "number") weeklyDrills.value.target = data.streak.week_goal;
    }
    ...
    const drillsRaw = Array.isArray(data.recent_drills) ? data.recent_drills : [];
    recentDrills.value = drillsRaw.map((d, index) => {
      ...
      let score = typeof d.score === "number" ? d.score : Number(d.score ?? NaN);

      if (!Number.isFinite(score) && total > 0) score = Math.round((correct / total) * 100);

      return {
        id: d.id ?? index,
        courseCode: d.course_code || d.courseCode || "COURSE",
        courseTitle: d.course_title || d.courseTitle || d.title || "Drill session",
        score: score || 0,
        correct,
        total,
